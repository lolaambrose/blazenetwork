...
async def start(message: types.Message):
    ...
    if not user:
        ...
        if coupon:
            ...
            if coupon_data:
                ...
                logger.info(
                    f"user {message.chat.id} activated coupon {coupon} with value ${balance}"
                )
                bot.send_message(
                    message.chat.id,
                    f"<b>Купон <code>{coupon}</code> успешно активирован!\n</b>💰 Ваш баланс пополнен на <b>${balance}</b>",
                )
            else:
                await bot.send_message(
                    message.chat.id,
                    f"<b>Купон <code>{coupon}</code> не найден или уже недействителен.</b>",
                )
                logger.info(
                    f"user {message.chat.id} tried to activate invalid coupon {coupon}."
                )

        user = await UserService.init_user(
            message.chat.id,
            str(uuid.uuid4()),
            datetime.now(),
            referral_id,
        ...
        if coupon:
            ...
            if coupon_data:
                ...
                logger.info(
                    f"user {user.id} activated coupon {coupon} with value ${coupon_data['value']}"
                )
                await bot.send_message(
                    user.id,
    ...
    await message.reply(
        "Добро пожаловать в <b>🚀 blazeVPN</b>!\n\nВыберите действие",
...
async def menu_subscription(message: types.Message):
    ...
    for server in network.SERVERS:
        ...
        xui_instance = next(
            (
                instance
                for instance in network.XUI_INSTANCES
                if instance[2]["id"] == server["id"]
            ),
            None,
        )
        ...
        if (
            xui_instance and xui_instance[1]
        ):  # xui_instance[1] содержит значение is_logged_in
            kb += [
                [
                    InlineKeyboardButton(
                        text=server["name"], callback_data=f"connect_{server['id']}"
                    )
                ]
            ]
...
async def action_connect(query: types.CallbackQuery):
    ...
    for xui, is_logged_in, serverinfo in network.XUI_INSTANCES:
        ...
        if not config:
            bot.send_message(
                query.from_user.id,
                "<b>Ошибка при получении конфигурации сервера.</b>"
        ...
        with io.BytesIO() as output:
            ...
            await bot.send_message(
                query.from_user.id,
                text=f"Ваша ссылка для подключения к <b>{serverinfo['name']}</b>\n\n"
                f"<code>{config}</code>\n\n",
            )
...
async def menu_invite(query: types.CallbackQuery):
    ...
    referral_description = (
        "🤝 Пригласите друга и получите <b>бонусные дни</b> подписки!\n\n"
...
async def menu_buy(argument):
    ...
    if not await Utils.is_user_subscribed(message.chat.id):
        ...
        kb = [
            [
                InlineKeyboardButton(
                    text="🚀 Подписаться", url=f"tg://resolve?domain={channel}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🔄 Проверить подписку", callback_data="menu_buy_subscription"
                )
            ],
        ]
        await message.answer(
            "Привет! Я заметил, что ты еще не подписался на наш новостной канал.\n\n"
    ...
    for sub in SubService.POOL:
        kb += [
            [
                InlineKeyboardButton(
                    text=f"📅 Купить {sub['name_ru']} – ${sub['price']}",
                    callback_data=f"buy_{sub['id']}",
                )
            ]
        ]
...
async def action_prolongate(query: types.CallbackQuery):
    ...
    await query.message.answer(
        f"✅ Подписка <b>{sub_data['name_ru']}</b> успешно продлена на <code>{sub_data['duration']}</code> дней."
    )
...
async def action_buy_callback(query: types.CallbackQuery):
    ...
    if user_sub:
        kb = [
            InlineKeyboardButton(
                text="✅ Да, продлить", callback_data=f"menu_prolongate_{subscription}"
            )
        ]
    ...
    await query.message.answer(
        f"Вы собираетесь купить <b>{sub_data['name_ru']} за ${sub_data['price']}</b>\n\n"
...
async def action_confirm_buy(query: types.CallbackQuery):
    ...
    for admin in config.TELEGRAM_ADMINS:
        await bot.send_message(
            admin,
            f"<i>[ADMIN NOTIFY]</i> <b>Пользователь <code>{user.id}</code> купил подписку <code>{sub_data['name_ru']}</code> за <code>${sub_data['price']}</code>.</b>",
        )
...
async def menu_deposit(query: types.CallbackQuery):
    ...
    eth_address = next(
        (wallet["address"] for wallet in wallets if wallet["currency"] == "ETH"), None
    )
    usdt_address = next(
        (wallet["address"] for wallet in wallets if wallet["currency"] == "USDT"), None
    )
    btc_address = next(
        (wallet["address"] for wallet in wallets if wallet["currency"] == "BTC"), None
    )
    ltc_address = next(
        (wallet["address"] for wallet in wallets if wallet["currency"] == "LTC"), None
    )
...
async def menu_information(message: types.Message):
    ...
    kb = [
        [
            InlineKeyboardButton(
                text="🇷🇺 Служба поддержки", url="tg://resolve?domain=blazenetworksupp"
            )
        ]
    ]
...
class Utils:
    ...
    async def render_status(message: types.Message):
        ...
        for xui, is_logged_in, server_info in network.XUI_INSTANCES:
            ...
            status_msg += (
                "├ ⌛ Последний опрос <code>"
                + server_info["last_seen"].strftime("%H:%M %d/%m")
                + "</code>\n"
            )
            status_msg += "├ 🕑 Uptime <code>" + uptime + "</code>\n"
            status_msg += f"└ ⬆️ <code>{round(up)} Mbytes</code> ⬇️ <code>{round(down)} Mbytes</code>\n\n"

        active_subs = len(await SubService.get_all_active())

        status_msg += "<b>👥 Пользовательская статистика</b>\n"
        status_msg += (
            f"├👤 Пользователей всего <code>{await UserService.count()}</code>\n"
        )
        status_msg += f"└🔒 Активных подписок <code>{active_subs}</code>\n"
    ...
    async def render_profile(user: User, chat_id: int = None, admin: bool = False):
        ...
        await bot.send_message(
            chat_id, profile_info, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb)
        )
                ...
                kb += [
                    [
                        InlineKeyboardButton(
                            ...
                            callback_data="menu_buy_subscription",
                        )
                    ]
                ]
...
class Admin:
    ...
    async def add_referal_days(user: User, days: int):
        ...
        if not sub:
            custom_sub = {"name": "Бонусная подписка", "price": 0.00, "duration": days}
        ...
        await UserService.upsert(user)
        await bot.send_message(
            user.id, f"🎉 Вам начислено <b>{days}</b> бонусных дней за реферала."
        )

    @staticmethod
    async def add_subscription(
        user: User, sub_data: dict, notify: bool = True
    ) -> Subscription:
        user_sub = await user.get_active_sub()
        ...
        logger.info(
            f"user {user.id} has bought a subscription {sub_data['name_ru']} for ${sub_data['price']}."
        )

        if notify:
            await bot.send_message(
                user.id, f"✅ Подписка <b>{sub_data['name_ru']}</b> успешно куплена\n"
            )
    ...
    async def remove_subscription(sub: Subscription):
        ...
        if user:
            ...
            await bot.send_message(
                user.id, f"❌ Ваша подписка <b>{sub.plan}</b> закончилась"
            )
        else:
    ...
    async def add_balance(user: User, amount: float, notify: bool = True):
        ...
        if notify:
            await bot.send_message(
                user.id, f"💰 Ваш баланс успешно пополнен на <b>${amount}</b>"
            )
    ...
    async def set_balance(user: User, amount: float):
        ...
        await UserService.upsert(user)
        await bot.send_message(
            user.id, f"💰 Ваш баланс успешно обновлен на <b>${amount}</b>"
        )
    ...
    async def command_set_balance(message: types.Message, **kwargs):
        ...
        await message.answer(
            f"Баланс пользователя {user_id} успешно обновлен на ${amount}"
        )
    ...
    async def command_add_sub(message: types.Message, **kwargs):
        ...
        sub_data = {
            ...
            "duration": [sub["duration"] for sub in SubService.POOL if sub["id"] == id][
                0
            ],
        }
        ...
        await message.answer(
            f"Подписка <b>{name}</b> успешно добавлена для ID <code>{user_id}</code>."
        )
        logger.info(
            f"[ADMIN] subscription {id} added for {user.id} by {message.from_user.id}"
        )
    ...
    async def command_remove_sub(message: types.Message, **kwargs):
        ...
        logger.info(
            f"[ADMIN] subscription removed for {user.id} by {message.from_user.id}"
        )
    ...
    for xui, is_logged_in, server_info in xui_instances:
        if not is_logged_in:
            ...
            for admin in config.TELEGRAM_ADMINS:
                await bot.send_message(
                    admin,
                    f"<i>[ADMIN NOTIFY]</i> <b>❌ server {server_info['full_address']} is not responding.</b>",
                )
...
async def notify_expiring_subs():
    ...
    kb = [
        InlineKeyboardButton(
            text="💳 Продлить подписку", callback_data="menu_buy_subscription"
        )
    ]

    subscriptions = (
        await SubService.get_expiring_subs()
    )  # Получаем все подписки, у которых дата окончания подходит

    for subscription in subscriptions:
        user = (
            await subscription.get_user()
        )  # Получаем пользователя для каждой подписки
        days_left = (
            subscription.datetime_end - datetime.now()
        ).days  # Вычисляем, сколько дней осталось до конца подписки
        if days_left in [1, 5, 0]:
            if days_left != 0:
                await bot.send_message(
                    user.id,
                    f"⏳ У вас осталось <b>{days_left} дней</b> до конца подписки.",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[kb]),
                )
            else:
                await bot.send_message(
                    user.id,
                    f"⏳ У вас остался <b>последний день</b> подписки.\n Пожалуйста, продлите подписку, чтобы не потерять доступ к сервису.",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[kb]),
                )
