...
async def create_wallet(
    currency, network, order_id, url_callback=None, from_referral_code=None
):
    data = {"currency": currency, "network": network, "order_id": str(order_id)}
    ...
    # Генерация подписи
    sign = hashlib.md5(
        (base64.b64encode(encoded_data.encode()) + config.MERCHANT_API_KEY.encode())
    ).hexdigest()
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            config.MERCHANT_API_URL + "/wallet", headers=headers, data=encoded_data
        ) as response:
            if response.status == 200:
                ...
                raise Exception(
                    f"Ошибка создания статичного кошелька: {response.status}, Тело ответа: {response_text}"
                )
                return None
